...
def find_transition_points(filename, num_slides, transition_duration=1.5, fade_duration=3.0):
    ...
    if len(transition_points) < num_slides + 1:
        print("not enough candidates, using evenly spaced points")
        transition_points = np.linspace(duration / (num_slides + 1), 
                                      duration - fade_duration, num_slides+1)
...
def analyze_audio_new(filename, num_slides, transition_duration=1.5, fade_duration=3.0):
    """
    ...
    Args:
        ...
        fade_duration: Duration of fade to black for final slide (seconds)
    """
    result_json = find_transition_points(filename, num_slides, transition_duration=transition_duration, fade_duration=fade_duration)
    print(result_json)
    # fname, _ = os.path.splitext(filename)
    # fname = os.path.join(os.path.dirname(filename), os.path.basename(fname) + "_slide_transitions.json")
    # json.dump(result_json, open(fname, "w"), indent=4)
    return result_json
